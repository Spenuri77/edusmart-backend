...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    // Zapytanie SQL
    $query = "
        SELECT marks.id, marks.mark, marks.date, marks.semester, marks.type, marks.description, marks.subject_id, subjects.subject_name, CONCAT(students.first_name, ' ', students.surname) as student_name, classes.class_name FROM `marks` JOIN students ON marks.student_id = students.id JOIN classes ON marks.class_id = classes.id JOIN subjects ON marks.subject_id = subjects.id WHERE teacher_id = $teacher_id ORDER BY date DESC LIMIT 10;
